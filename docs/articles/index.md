<div id="main" class="col-md-9" role="main">

# Articles

<div class="section">

### Learn Two-Sample MR

<div class="section-desc">

</div>

-   [Course 1: Why Mendelian Randomization?](Course01-WhyMR.md):
-   [Course 2: Instrumental Variables and MR
    Assumptions](Course02-InstrumentalVariablesAndMRAssumptions.md):
-   [Course 3: The Mechanics of Two-Sample
    MR](Course03-TwoSampleMRMechanics.md):
-   [Course 4: Estimation and Sensitivity
    Analysis](Course04-EstimationAndSensitivityAnalysis.md):
-   [Course 5: From Theory to the Medusa
    Workflow](Course05-FromTheoryToMedusaWorkflow.md):
-   [Course 6: Doing and Critiquing a Two-Sample MR
    Study](Course06-DoingAndCritiquingATwoSampleMRStudy.md):

</div>

<div class="section">

### Practical Guides

<div class="section-desc">

</div>

-   [Getting Started with Medusa](GettingStarted.md):
-   [IL-6 Signaling and Colorectal Cancer: A Complete MR
    Walkthrough](ColorectalCancerIL6Example.md):
-   [Negative Control Outcomes and Empirical
    Calibration](NegativeControlCalibration.md):
-   [Federated Analysis Guide for Network
    Coordinators](FederatedAnalysisGuide.md):

</div>

</div>
