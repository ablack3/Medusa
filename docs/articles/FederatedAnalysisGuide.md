...
substitute its own connection details, cohort IDs, file paths, and
governance controls.
...
identify individuals. If a network wants IVW, MR-Egger, or
weighted-median sensitivity analyses, each site can also share one row
per SNP containing `beta_ZY` and `se_ZY`. Those are still aggregate
summaries, but they are optional and separate from the main pooled
likelihood workflow.
...
perSnpBySite <- list(
  site_A = read.csv("medusa_per_snp_site_A.csv", stringsAsFactors = FALSE),
  site_B = read.csv("medusa_per_snp_site_B.csv", stringsAsFactors = FALSE),
  site_C = read.csv("medusa_per_snp_site_C.csv", stringsAsFactors = FALSE)
)
...
The required profile CSV files are human-readable and typically very
small (&lt; 100 KB) because they contain only numeric grid values and
summary statistics. Optional per-SNP summary CSVs are also compact.
Using CSV ensures that every value leaving a site can be inspected and
audited before transfer.
...
-   Every imported profile has the expected site identifier and non-zero
    case count.
-   If per-SNP summaries were requested, each file uses the same SNP IDs
    and allele coding.
