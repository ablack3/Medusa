...
instrumentTable <- readRDS("instruments.rds")
...
# ONLY these files leave the site:
saveRDS(profile, file = sprintf("medusa_profile_%s.rds", siteId))
saveRDS(diagnostics$diagnosticFlags,
        file = sprintf("medusa_diagnosticFlags_%s.rds", siteId))

message("Analysis complete. Share the RDS files with the coordinator.")
```

## Coordinator Pooling Script

After collecting profile RDS files from all sites:
...
# Load site profiles
siteProfiles <- list(
  site_A = readRDS("medusa_profile_site_A.rds"),
  site_B = readRDS("medusa_profile_site_B.rds"),
  site_C = readRDS("medusa_profile_site_C.rds")
)
...
Profile RDS files should be transferred securely between sites and coordinator.
Options include:
...
The RDS files are typically very small (< 100 KB) as they contain only numeric
