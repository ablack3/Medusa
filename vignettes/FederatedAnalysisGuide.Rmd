...
site-level summary artifacts leave each site** — no individual-level data is
shared. The primary artifact is the profile log-likelihood vector, accompanied
by site metadata and the allele-score definition needed to verify that every
site fit the same score.
...
an MLE at the grid boundary, plus the fixed allele-score definition so the
coordinator can verify consistency across sites.
...
Each site needs the **VARIANT_OCCURRENCE** table from the
[OMOP Genomic CDM](https://github.com/OHDSI/Genomic-CDM).
This table stores per-person variant calls. The minimal required columns are:
...
If the genomic extension tables are in a different schema from the main CDM,
specify it via the `genomicDatabaseSchema` parameter (defaults to
`cdmDatabaseSchema`).
